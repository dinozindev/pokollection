  ...
  return (
    <section className="flex items-center py-30 flex-col min-h-screen">
      ...
      <div className="flex gap-4 flex-wrap justify-center w-full">
        {binders?.map(binder => (
          <Link to={`/binders/${binder.id}`} className="bg-white p-6 w-1/2 lg:w-1/4 h-110 md:h-110 lg:h-120 flex flex-col items-center gap-4 cursor-pointer hover:-translate-y-2 transition-all rounded-xl" key={binder.id}>
