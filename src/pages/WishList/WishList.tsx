...
const WishList = () => {
    ...
    useEffect(() => {
        if (!user) return;
        const cardsRef = collection(db, "users", targetUid, "wishlist");
        ...
        <section className="flex items-center py-30 flex-col min-h-screen">
            ...
            <div className={`flex flex-wrap justify-center gap-6 ${wishlistedCards.length <= 2 ? "w-full" : ""}`}>
                {wishlistedCards.length !== 0 ? wishlistedCards?.map(card => (
                    isOwnProfile ?
                        <CardDiv key={card.id} loadedImages={loadedImages} card={card} handleImageLoad={handleImageLoad} wishlist={wishlist} addToBinder={!!user} toggleWishlist={toggleWishlist}/> : <CardDiv key={card.id} loadedImages={loadedImages} card={card} handleImageLoad={handleImageLoad} wishlist={wishlist} />
